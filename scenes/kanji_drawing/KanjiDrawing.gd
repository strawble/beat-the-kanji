extends CanvasLayer
# KanjiDrawing — parry overlay. Extends CanvasLayer so it always renders
# on top of all scene content regardless of where it is added.
# Layer 50: above gameplay (0), below NavBar (100).

signal drawing_validated(success: bool, coverage: float)

const CANVAS_SIZE        : Vector2 = Vector2(600, 900)
const STROKE_WIDTH       : float   = 12.0
const COVERAGE_THRESHOLD : float   = 0.40
...
var coverage_bar  : ProgressBar = null
...
func _build_ui() -> void:
	...
	root.color = Color(0.08, 0.08, 0.12, 0.97)
	...
	title_label.text = "⚠  The boss is about to attack!"
	title_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title_label.add_theme_font_size_override("font_size", 20)
	title_label.add_theme_color_override("font_color", Color(1.0, 0.45, 0.35))
	...
	# Watermark guide label (big kanji in background)
	...
	# Coverage feedback bar
	coverage_bar = ProgressBar.new()
	coverage_bar.custom_minimum_size = Vector2(CANVAS_SIZE.x, 10)
	...
	validate_btn.custom_minimum_size = Vector2(0, 48)
	...
	give_up_btn.custom_minimum_size = Vector2(0, 48)
...
func _update_ui() -> void:
	...
	guide_label.text = char_str

	match current_attempt:
		1:
			guide_label.modulate = Color(0.4, 0.6, 1.0, 0.85)
			hint_label.text = "✏️  Full guide — trace over the model"
		2:
			guide_label.modulate = Color(0.4, 0.6, 1.0, 0.40)
			var stroke_count : int = current_kanji.get("stroke_count", 4)
			hint_label.text = "✏️  %d strokes — complete from memory" % stroke_count
		3:
			guide_label.modulate = Color(0.4, 0.6, 1.0, 0.15)
...
func _on_canvas_draw() -> void:
	for stroke in _all_strokes:
		...
		for i in range((stroke as Array).size() - 1):
			canvas_node.draw_line(stroke[i], stroke[i + 1],
				Color(1.0, 1.0, 1.0, 0.8), STROKE_WIDTH, true)
	if _current_stroke.size() >= 2:
		for i in range(_current_stroke.size() - 1):
			canvas_node.draw_line(_current_stroke[i], _current_stroke[i + 1],
				Color(1.0, 1.0, 1.0, 0.8), STROKE_WIDTH, true)
...
func _on_validate_pressed() -> void:
	...
	if success:
		hint_label.text = "✨  Validated! (coverage: %d%%)" % int(coverage * 100)
		await get_tree().create_timer(0.6).timeout
		emit_signal("drawing_validated", true, coverage)
	else:
		if current_attempt < max_attempts:
			...
			hint_label.text = "❌  Attempts exhausted (%d%%)" % int(coverage * 100)
...
func _compute_coverage() -> float:
	...
	for stroke in all:
		for pt in (stroke as Array):
			var cell := Vector2i(
				clamp(int(pt.x / (CANVAS_SIZE.x / 4.0)), 0, 3),
				clamp(int(pt.y / (CANVAS_SIZE.y / 4.0)), 0, 3)
			)
			grid[cell] = true
